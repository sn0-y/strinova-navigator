...
export class WeeklyModReportTask extends ScheduledTask {
    public async run() {
        ...
        try {
            ...
            const formatLine = (r: any) => `<@${r.userId}> — Tier ${r.oldTier} ➔ **Tier ${r.newTier}** (Score: **${r.score}**)\n*Cases: ${r.stats.cases} | Tickets: ${r.stats.tickets}*`;

            if (promotions.length > 0) embed.addFields({ name: '⬆️ Promotions', value: promotions.map(formatLine).join('\n\n') });
            if (demotions.length > 0) embed.addFields({ name: '⬇️ Demotions (Neglect Detected)', value: demotions.map(formatLine).join('\n\n') });
            
            if (maintained.length > 0) {
                const maintainedStr = maintained.map(formatLine).join('\n\n');
                embed.addFields({ name: '➖ Maintained Tier', value: maintainedStr.length > 1024 ? maintainedStr.substring(0, 1000) + '...\n*(Truncated)*' : maintainedStr });
            }
