...
failed++;
reportRows.push(
this.buildReportRow(lineNumber, trimmed, '', 'invalid-input', `Could not extract a valid ${parseType === 'user-id' ? 'user ID' : 'username'}`)
);
continue;
}
...
reportRows.push(this.buildReportRow(lineNumber, extractedValue, member.id, 'error', 'Failed to assign role (permissions or hierarchy)'));
}
...
const cleaned = username.trim().replace(/^"+|"+$/g, '').replace(/^@/, '');
